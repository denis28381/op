function Spin2WinWheel()
{
  ...
  var xmlns = 'http://www.w3.org/2000/svg',
    ...
    toast = select('.toast'),
    toastText = select('.toast .toast__inner'),
    ...
    setInitData = function ()
    {
      ...
      randomSpins = dataObj.randomSpins;
    ...
    throwUpdate = function (e)
    {
      ...
      if (currentWheelPos != oldWheelPos)
      {
        ...
        var rotation = (currentWheelPos > oldWheelPos) ? -30 : 30;

        TweenMax.fromTo(peg, 0.5, {
              ...
              ThrowPropsPlugin.to(wheel, {
                throwProps: {
                  rotation: {
                    ...
                    //if it's random spins then get a random spin but pass in the multiplier to ensure a long spin (plus the right slot id)
                    //if it has destinations set then use those
                    end: (randomSpins) ? getRandomClickSpin() : winItem
                    //end:dest
                  }
                },
                onStart: onButtonPress,
    ...
    endGame = function (show_message = true)
    {
      ...
      if (show_message)
      {
        ...
        TweenMax.to(toastText, 1, {
          text: typeof gameOverText === 'string' ? gameOverText : '',
          ease: Linear.easeNone,
          delay: 2
        });

        if (typeof gameOverText === 'function')
        {
          gameOverText();
        }
      }
    ...
    {
      spinButton = document.querySelector(spinTrigger);
  ...
  this.init = function (e)
  {
    ...
    onStart = (e.onStart) ? e.onStart : () => new Promise(() =>
    {
    });
    ...
    spinTrigger = e.spinTrigger ? e.spinTrigger : null;
  };

  this.setWinItem = (item) =>
  {
    winItem = -(rotationStep * item) - numRevsPerDestination * spinMultiplier;
  };
